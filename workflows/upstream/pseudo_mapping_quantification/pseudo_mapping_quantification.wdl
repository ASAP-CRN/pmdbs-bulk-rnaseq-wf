...
task mapping_quantification {
	...
	command <<<
		...
		tar -czvf "~{dataset_id}.~{sample_id}.mapping_mode.salmon_quant.tar.gz" "~{sample_id}_salmon_quant"

		upload_outputs \
			-b ~{billing_project} \
			-d ~{raw_data_path} \
			-i ~{write_tsv(workflow_info)} \
			-o "~{dataset_id}.~{sample_id}.mapping_mode.salmon_quant.tar.gz"
	>>>

	output {
		String quant_tar_gz = "~{raw_data_path}/~{dataset_id}.~{sample_id}.mapping_mode.salmon_quant.tar.gz"
	}
