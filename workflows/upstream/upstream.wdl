...
workflow upstream {
	...
	scatter (sample_index in range(length(samples))) {
		...
		String fastqc_raw_reads_reports_tar_gz = "~{fastqc_raw_reads_raw_data_path}/~{dataset_sample_id}.fastqc_reports.tar.gz"
		...
		String fastp_failed_paired_fastqs = "~{fastp_raw_data_path}/~{dataset_sample_id}.fastp_failed_paired_fastqs.tar.gz"
		String fastp_reports_html = "~{fastp_raw_data_path}/~{dataset_sample_id}.fastp_reports.tar.gz"
		String fastp_jsons = "~{fastp_raw_data_path}/~{dataset_sample_id}.fastp_json.tar.gz"
		...
		String fastqc_trimmed_reads_reports_tar_gz = "~{fastqc_trimmed_reads_raw_data_path}/~{dataset_sample_id}.trimmed_fastqc_reports.tar.gz"
		...
		if (run_alignment_quantification) {
			String star_aligned_bam = "~{star_and_salmon_alignment_mode_raw_data_path}/~{dataset_sample_id}.Aligned.sortedByCoord.out.bam"
			String star_aligned_bam_index = "~{star_and_salmon_alignment_mode_raw_data_path}/~{dataset_sample_id}.Aligned.sortedByCoord.out.bam.bai"
			String star_aligned_to_transcriptome_bam = "~{star_and_salmon_alignment_mode_raw_data_path}/~{dataset_sample_id}.Aligned.toTranscriptome.out.bam"
			String star_unmapped_mate1 = "~{star_and_salmon_alignment_mode_raw_data_path}/~{dataset_sample_id}.Unmapped.out.mate1"
			String star_unmapped_mate2 = "~{star_and_salmon_alignment_mode_raw_data_path}/~{dataset_sample_id}.Unmapped.out.mate2"
			String star_log = "~{star_and_salmon_alignment_mode_raw_data_path}/~{dataset_sample_id}.Log.out"
			String star_final_log = "~{star_and_salmon_alignment_mode_raw_data_path}/~{dataset_sample_id}.Log.final.out"
			String star_progress_log = "~{star_and_salmon_alignment_mode_raw_data_path}/~{dataset_sample_id}.Log.progress.out"
			String star_sj_out_tab = "~{star_and_salmon_alignment_mode_raw_data_path}/~{dataset_sample_id}.SJ.out.tab"
			String salmon_alignment_mode_quant_tar_gz = "~{star_and_salmon_alignment_mode_raw_data_path}/~{dataset_sample_id}.alignment_mode.salmon_quant.tar.gz"
		...
		if (run_pseudo_mapping_quantification) {
			String salmon_mapping_mode_quant_tar_gz = "~{salmon_mapping_mode_raw_data_path}/~{dataset_sample_id}.mapping_mode.salmon_quant.tar.gz"
...
task trim_and_qc {
	...
	command <<<
		...
		tar -czvf "~{dataset_sample_id}.fastp_failed_paired_fastqs.tar.gz" "~{sample_id}_fastp_failed_paired_fastqs"

		mkdir -p ~{sample_id}_fastp_reports
		find . -maxdepth 1 -type f -name "*fastp.html" -exec mv {} ~{sample_id}_fastp_reports/ \;
		tar -czvf "~{dataset_sample_id}.fastp_reports.tar.gz" "~{sample_id}_fastp_reports"

		mkdir -p ~{sample_id}_fastp_json
		find . -maxdepth 1 -type f -name "*fastp.json" -exec mv {} ~{sample_id}_fastp_json/ \;
		tar -czvf "~{dataset_sample_id}.fastp_json.tar.gz" "~{sample_id}_fastp_json"

		upload_outputs \
			-b ~{billing_project} \
			-d ~{raw_data_path} \
			-i ~{write_tsv(workflow_info)} \
			-o "~{dataset_sample_id}.fastp_failed_paired_fastqs.tar.gz" \
			-o "~{dataset_sample_id}.fastp_reports.tar.gz" \
			-o "~{dataset_sample_id}.fastp_json.tar.gz"
	>>>
	...
	output {
		...
		String qc_failed_paired_fastqs_tar_gz = "~{raw_data_path}/~{dataset_sample_id}.fastp_failed_paired_fastqs.tar.gz"
		String qc_reports_html_tar_gz = "~{raw_data_path}/~{dataset_sample_id}.fastp_reports.tar.gz"
		String qc_json_tar_gz = "~{raw_data_path}/~{dataset_sample_id}.fastp_json.tar.gz"
	}
